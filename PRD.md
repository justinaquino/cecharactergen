...
- **Career Enable/Disable Toggle List**
  - All careers displayed as checklist
  - Default: All 24 careers enabled
  - GM can disable careers not in their setting (e.g., no Nobles in frontier campaign)
  - Filter by category: Military, Civilian, Criminal, Elite
  - "Enable All" / "Disable All" buttons
  - "Reset to Default" restores all careers
- **Career Visibility in Generation**
  - Disabled careers hidden from `/generate` dropdown
  - Disabled careers excluded from random generation
  - Visual indicator: grayed out with tooltip "Disabled in settings"
- **Per-Career Settings**
...
```json
{
  "careerPreferences": {
    ...
    "marine": { "enabled": true },
    "noble": { "enabled": false, "reason": "Not in frontier setting" },
...
2. **`careers.json`** — 24 career paths with full rules
   - **M2 Content:** First 6-8 careers converted to JSON (Drifter, Marine, Scout, etc.)
   - Each career has `enabled` field for GM control
   - Includes: qualification, survival, advancement, skills, ranks, mustering
   - Remaining careers added in M3
